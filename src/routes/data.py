from fastapi import FastAPI , APIRouter , Body , Depends, UploadFile , status , Request
...
async def upload_data(
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "signal": ResponseSingnals.FILE_UPLOAD_FAILED.value             
            }
        )
    ...
    return JSONResponse(
        content={
            "signal": ResponseSingnals.FILE_UPLOAD_SUCCESS.value,
            "file_id": str(asset_record.asset_id),
        }
    )

@data_router.post("/process/{project_id}")
async def process_data(request: Request,project_id: int,process_request: ProcessRequest = Body(...), settings: Settings = Depends(get_settings)):
    ...
    for asset_id, file_id in project_files_ids.items():
        ...
        if file_chunks is None or len(file_chunks) == 0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseSingnals.PROCESSING_FAIELD.value
                }
            )
